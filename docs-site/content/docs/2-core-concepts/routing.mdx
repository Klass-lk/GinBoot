---
title: "Routing"
description: "Organise endpoints with controllers and route groups, read path parameters, and pick between Ginboot's four handler signatures."
icon: Route
---
...
<ApiSignature signature="func(ctx *ginboot.Context) (T, error)">
  You need context utilities — auth, pagination, path params — but no request body.
</ApiSignature>

<ApiSignature signature="func(request R) (T, error)">
  You only need the parsed and validated request body.
</ApiSignature>

<ApiSignature signature="func() (T, error)">
  No input at all — health checks, static lookups, simple stats.
</ApiSignature>

<ApiSignature signature="func(ctx *ginboot.Context, request R) (T, error)">
